...
//Этот класс является подклассом FractalGenerator и используется для вычисления  BurningShip
public class BurningShip extends FractalGenerator {
    ...
    /**
     * Этот метод реализует итеративную функцию для фрактала BurningShip.
     */
    public int numIterations(double x, double y)
    {
        ...
        /**
         * Вычислите Zn = (abs[Re(zn-1)] + i(abs[im(zn-1)]))^2 + c, где значения представляют
         * собой комплексные числа, где Z0=0 (по реальному и мнимому),
         * а c-это конкретная точка в фрактале (задается x и y).
        ...
        while (iter < MAX_ITER && zreal * zreal + zimaginary * zimaginary < 4)
        {
            ...
            double zimaginary2 = 2 * Math.abs(zreal)  * Math.abs(zimaginary) + y;
            zreal = zreal2;
